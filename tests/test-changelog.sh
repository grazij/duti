...
for h in cf2020f 3e5c7de 7810364 b682406; do
...
#
#   0c21a8e..9c05250  ci: + docs: only            -> must not trigger
#   4b40008..9c05250  those plus untyped subjects -> must trigger, and the
#                                                    docs/ci commits must
#                                                    still be listed
WT="$TMP/wt"
if git worktree add --detach -q "$WT" 9c05250 >/dev/null 2>&1; then
	cp "$SCRIPT" "$WT/make-changelog.sh"

	echo "=== T7: docs/ci alone does not trigger a release ==="
	( cd "$WT" && ./make-changelog.sh 9.9.9-test CL7.md 0c21a8e ) \
		>"$TMP/t7.out" 2>&1
	rc=$?
	...
	( cd "$WT" && ./make-changelog.sh 9.9.9-test CL8.md 4b40008 ) \
		>"$TMP/t8.out" 2>&1
	rc=$?
	if [ "$rc" -ne 0 ]; then
		bad "expected exit 0 for a range containing untyped commits, got $rc"
	else
		# 9c05250 is ci:, 1545d27 and 7f4b44f are docs:
		absent=""
		for h in 9c05250 1545d27 7f4b44f; do
	...
	bad "could not create a worktree at 9c05250 (shallow clone?)"
fi
